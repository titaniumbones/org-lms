...
Much of the code in this library reads and sets properties at the headline level.  This code should increase readability and maintainability by farming those functions out to reusable helpers; these values can then be extracted with `let-alist`. 
