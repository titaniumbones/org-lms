  ...
  (let* ((assid
    ...
    (cl-loop for attachment in (plist-get submission :attachments)
             ...
             (let* ((downloadurl (plist-get attachment :url))
                    ...
                    (r (request downloadurl
                                :sync t
                                :encoding 'no-conversion
                                :parser 'buffer-string
                                :success (cl-function
                                          (lambda (&key data &allow-othr-keys)
                                            (message "success!")
                                            data))
                                :error (cl-function (lambda ( &key error-thrown data status &allow-other-keys )
                                                      (setq canvas-err error-thrown)
                                                      (message "ERROR: %s" error-thrown)))))
                    (f (request-response-data r))
                    (e (request-response-error-thrown r))
                    (filename (expand-file-name filename (org-entry-get (point) "ORG_LMS_ASSIGNMENT_DIRECTORY")))
